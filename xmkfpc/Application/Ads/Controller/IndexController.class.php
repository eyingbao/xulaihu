...
class IndexController extends Controller {
	...
	FORMAT(  SUM(COST) / SUM(CLICKS) ,2)
			...
			FROM `aw_reportkeywords` WHERE `ACCOUNT_ID` = {$this->cid} AND DAY 
			BETWEEN '{$this->d1}' AND '{$this->d2}'  AND NETWORK = 'Search Network'  GROUP BY CRITERIA, CAMPAIGN_NAME,KEYWORD_MATCH_TYPE) AS new ORDER BY new.COST DESC LIMIT 0 ,5";
			$histogram1List = M('reportcampaign','aw_',$this->connectReports)->query($sql2);*/
	...
	public function customerService(){
		...
		$salesOrder_Id= M('n_accountinfo',NULL)->where($map)->getfield("salesOrder_Id");
		$w['id']=$salesOrder_Id;
		$responsibleForCustomerService= M('n_salesorder',NULL)->where($w)->getfield("responsibleForCustomerService");
		$w['id']=$responsibleForCustomerService;
		$userInfo= M('n_userprofile',NULL)->where($w)->find();
		$userInfo = array();
	...
	public function customer(){
		...
		$mccID = substr($this->cid,0,3).'-'.substr($this->cid,3,3).'-'.substr($this->cid,6,4);
		/*$map['account_Id']=$mccID;
		$fromUserId= M('n_accountinfo',NULL)->where($map)->getfield("customer_Id");	
		$w['id']=$fromUserId;
		$orderInfo= M('n_customerinformation',NULL)->where($w)->find();
		$orderInfo = array();
