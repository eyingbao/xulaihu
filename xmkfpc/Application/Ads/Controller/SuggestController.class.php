...
class SuggestController extends Controller{
	...
	//（分值标准）3优 2良 1差
	protected $sta =  array(
		'costErr'=>array(-5,5,10), 		//（账户端）消耗异常
		
		'renewals'=>array(-5,5,10),	//（账户端）续费提醒
		
		'budgetErr'=>array(-3,5,8),	//（账户端）日预算预警
		
		'convers'=>array(-3,0,8),		//（广告系列）转化次数对比
		
		'delivery'=>array(-3,5,8),		//（广告系列）投放渠道
		
		'campaignCtr'=>array(-3,5,10), //（广告系列）平均点击率
		
		'kwPoint'=>array(-5,10,15),	//（广告组）关键词质量得分
		
		'kwCount'=>array(-3,5,8),	//（广告组）关键词数量
		
		'ref'=>array(-3,0,3),	//（广告组）广告语状态
		
		'adCount'=>array(-3,0,3),	//（广告组）广告语条数
		
		'BounceRate'=>array(-5,3,5),	//（GA网站数据）跳出率
		
		'Loadtime'=>array(-5,5,7),	//（GA网站数据）加载时间
		
		'Dwelltime'=>array(-5,3,5),	//（GA网站数据）停留时间
		
		'crmServerCount'=>array(-5,3,5),	//（联系频次）CRM库服务记录
	...
	public function renewals(){
		...
		$rsa =  M('budget')->where("cid = '{$cid}'")-> find();
...
(SELECT COUNT(*) AS C FROM `aw_reportkeywords` WHERE ACCOUNT_ID = '{$cid}' AND DAY BETWEEN '{$d1} 20:00:00' AND '{$d2} 20:00:00' AND NETWORK  = 'Search Network' AND STATUS = 'enabled' ) AS b";
	...
	public function BounceRate(){
		...
		$d1 = date("Y-m-d",strtotime("-7 day"));
		$cid = $_GET['cid'];
		$list = M('ga')->where("account = ".$cid)->select();
		$ua = $list[0];
	...
	public function converAndGa(){
		...
		$c1 = M('ads_conversions')->where("account = '{$cid}'")->count();
		$c2 = M('ads_ga')->where("account = '{$cid}'")->count();
