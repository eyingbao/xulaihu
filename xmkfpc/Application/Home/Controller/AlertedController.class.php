	...
	public function index(){
		...
		switch($type){
			case 'xftx': //续费提醒
				...
				$rsa =  M('budget2','ads_',$this->connectReports2)->where("cid = '{$cid}'")-> find();
...
AS rp ON rp.CAMPAIGN_ID = NEW.CAMPAIGN_ID";
			...
			case 'kwsl': //关键词数量
				...
				SELECT ADGROUP_NAME, CAMPAIGN_NAME ,CRITERIA ,COUNT(CRITERIA) AS C  FROM aw_reportkeywords WHERE DAY between '{$d1}' AND '{$d2}' AND ACCOUNT_ID = '{$cid}' AND  NETWORK = 'Search Network' AND STATUS = 'enabled'  GROUP BY  CRITERIA 
	...
	public function getFinfo(){
		...
		$arr['cname'] = M('mcc')->where("customerId = '{$_POST[cid]}'")->getField('name');
