...
class IndexController extends CommonController {
   ...
   public function index(){
	  ...
	  // exit;
	   ...
	   switch($mcc){
		   case 0: //总权限
		   		//echo 3;
				$mccer =  M('Mcc')->where('pid = 0 AND canManageClients =1')->find();
		   		$list = M('Mcc')->where('level = 1 AND canManageClients =1 AND status = 1 ')->select();
		   break;
		   
		   default: //其他权限		  
		    //echo 2;
			//exit; 
		  	 $mccer =  M('Mcc')->where("canManageClients =1 AND customerId = '{$mcc}'")->find();
			 $list = M('Mcc')->where("pid = '{$mcc}' AND status = 1")->select();
   ...
   public function downs(){
	   //http://adwords.cn/api/examples/AdWords/v201502/Reporting/DownloadCriteriaReportWithAwql.php
		...
		$list = M('Mcc')->where('canManageClients = 0')->field('customerId')->select();
	  //	print_r($list);
		//exit;
