...
class MccController extends CommonController {
	...
	public function getMcc(){
		...
		echo M('mcc')->where("customerId = '{$customerId}'")->getField('name');
	}
		...
		if(IS_POST){
			...
			if(is_array($_POST['ids']) && !empty($_POST['ids'])){
				...
				SELECT * FROM ads_budget  WHERE cid IN  ({$ids})
					...
					SELECT ne3.COST AS cost ,ne3.ACCOUNT_ID FROM ( SELECT SUM(COST) AS COST, ACCOUNT_ID FROM `aw_reportaccount` WHERE DAY BETWEEN '{$day1}' AND '{$day2}' GROUP BY ACCOUNT_ID ) AS ne3 WHERE ne3. ACCOUNT_ID IN ({$ids}) 
				
				
				) AS b ON a.cid = b.ACCOUNT_ID INNER JOIN (
				
				
				SELECT ne4.AVG_COST ,ne4.ACCOUNT_ID FROM ( SELECT AVG(COST) AS AVG_COST, ACCOUNT_ID FROM `aw_reportaccount` WHERE DAY BETWEEN '{$day1}' AND '{$day2}' GROUP BY ACCOUNT_ID ) AS ne4 WHERE ne4. ACCOUNT_ID IN ({$ids}) 
...
exit;*/
	...
	public function tools(){
		if(IS_POST){
			...
			$quick = array(
				...
				'ua'=>get_ua(NULL,$ids),
				'conversions'=>get_conversions(NULL,$ids)
			);
	...
	public function alertedList(){
		if(IS_GET){
			...
			$list2 = M('mcc')->where("account_id IN ($ids)")->getField('account_id,name',true);
			$list = M('ads_account_rating',NULL,$this->connectReports)->query($sql);
			foreach($list as $k=>$v){
