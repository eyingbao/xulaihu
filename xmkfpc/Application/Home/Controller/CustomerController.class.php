...
class CustomerController extends Controller {
	...
	public function _initialize(){
		...
		switch($sel){
			...
			case 1: //精准搜索
				...
				$mccIdList = M('mcc')->where('status = 1  AND canManageClients = 0')->field('id,customerId')->select(); //所有活跃非管理员账户
				$arr1= array();
				...
				if(!in_array($this->mcc,$farr)){
					...
					$rs = M('mcc')->where("customerId ='{$this->mcc}'")->find();
			...
			case 2: //模糊搜素
				...
				//$mccIdList = M('mcc')->where("status = 1  AND canManageClients = 0 AND name LIKE '%秦工%'")->field('customerId')->select(); //所有活跃非管理员账户
				$this->mcc = $_GET['mcc'];
				$mccList = M('mcc')->where('status = 1')->select(); //所有活跃账户
				findChildren($mccList,$_SESSION['mcc']['id'],$this->children); //获取当前登录账户下的所有子活跃账户
				$mccIdList = M('mcc')->where('status = 1  AND canManageClients = 0')->field('customerId')->select(); //所有活跃非管理员账户
				$arr1= array();
				foreach($mccIdList as $v){
					array_push($arr1,$v['customerid']);
				}
				...
				$selArr =  M('mcc')->where("status = 1  AND canManageClients = 0 AND name LIKE '%{$_GET[title]}%'")->getField('id,customerId'); 
	...
	public function getList(){
		...
		$list =  M('mcc')->where("status = 1  AND canManageClients = 0 AND id IN ({$ids})")->field('name,customerId')->select(); 
		
		//print_r("status = 1  AND canManageClients = 0 AND id IN ({$ids})");
	//	exit;
